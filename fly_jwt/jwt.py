...
def __fly_jwt(request):
    __conf = __fly_jwt._jwt_config
    __auth = __conf.auth_handler
    __func = __conf.success_handler
    if __func is None:
    ...
    try:
        ...
        for item in request["header"]:
            if item["name"] == "Authorization" or \
                ...
                encoded = __re_bearer(item["value"])
                if encoded is None:
                    raise Exception
                decoded = jwt.decode(encoded)
                if not __auth(decoded):
                    raise Exception
            break
        ...
        print(e)
        __fail = __conf.fail_handler
        if __fail is None:
            # TODO: return 401 return
            return None
        return __fail()
